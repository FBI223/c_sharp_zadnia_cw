...
public class Program
{
    public static void Main()
    {
        ...
        PrintMatrix(complexMultiplyResult);
    }
...
public class Matrix<T> where T : IComparable, IFormattable
{
    ...
    public Matrix<T> MultiplyMatrix(Matrix<T> other)
    {
        ...
        for (int i = 0; i < Rows; i++)
        {
            ...
            {
                dynamic first = this[i, 0] * other[0, j];
...
public class Complex<U> : IComparable, IFormattable where U : struct, IComparable, IFormattable
{
    ...
    // Operator dodawania dla Complex<U> umożliwiający dodawanie elementów Complex<U>
    public static Complex<U> operator +(Complex<U> a, Complex<U> b)
    {
        dynamic realA = a.Real;
        dynamic realB = b.Real;
...
public class Matrix<T> where T : IComparable, IFormattable
{
    ...
    // Dodawanie dwóch macierzy bez użycia Activator
    public Matrix<T> AddMatrix(Matrix<T> other)
    {
        if (Rows != other.Rows || Columns != other.Columns)
            throw new ArgumentException("Matrices must have the same dimensions.");
        ...
        for (int i = 0; i < Rows; i++)
        {
            for (int j = 0; j < Columns; j++)
            {
                dynamic a = this[i, j];
                dynamic b = other[i, j];
                result[i, j] = a + b; // Obsłuży typ Complex<U> oraz zwykłe typy numeryczne
            }
        }
        return result;
    }
...
public Matrix<T> MultiplyMatrix(Matrix<T> other)
{
    ...
    for (int i = 0; i < Rows; i++)
    {
        for (int j = 0; j < other.Columns; j++)
        {
            // Zamiast inicjalizować sum, zaczynamy od pierwszego elementu
            dynamic first = this[i, 0] * other[0, j];
            for (int k = 1; k < Columns; k++)
            {
                dynamic a = this[i, k];
                dynamic b = other[k, j];
                first += a * b; // Dodajemy kolejne iloczyny
            }
            result[i, j] = first;
        }
    }
